...
import com.dataqin.media.model.DiskStateModel;
...
public class SdCardUtil {
    ...
    public static long getInnerSDAvailableSize(Context context) {
        ArrayList<SDCardStat> sdCardStatList = getSDCardStats(context);
    ...
    public static ArrayList<SDCardStat> getSDCardStats(Context context) {
        ArrayList<SDCardStat> list = new ArrayList<>();
        try {
    ...
    //根据设备挂载次序排序SDCard
    private static ArrayList<SDCardStat> sortSDCardList(ArrayList<SDCardStat> list) {
        ArrayList<SDCardStat> resultList = new ArrayList<>();
        int minIdx = 0;
        for (SDCardStat stat : list) {
            if (minIdx == 0) {
                resultList.add(stat);
    ...
    private static DiskStateModel getDiskCapacity(String path) {
        ...
        return new DiskStateModel(blockSize * feeBlockCount, blockSize
                * totalBlockCount);
    }
    ...
    public static class SDCardStat {
        ...
        public static int SDCARD_MAX_COUNT = 1024;// voldMinorIdx 小于此值的为sdcard内置设备，大于此值的是u盘
