...
import com.ow.basemodule.widget.dialog.AndDialog
            ...
            AndPermission.with(context).runtime().permission(*groups).onGranted {
                ...
                if (permissions.isNotEmpty()) {
                    ...
                    when (permissionIndex) {
                        0 -> result = context.getString(R.string.permissions_camera_txt)
                        1 -> result = context.getString(R.string.permissions_microphone_txt)
                        2 -> result = context.getString(R.string.permissions_storage_txt)
                    }
                }

                //如果用户拒绝了开启权限
                if (AndPermission.hasAlwaysDeniedPermission(context, permissions)) {
                    AndDialog.show(context, context.getString(R.string.dialog_title_txt), MessageFormat.format(context.getString(R.string.dialog_permission_txt), result), context.getString(R.string.dialog_sure_txt), context.getString(R.string.dialog_cancel_txt), object : OnAndDialogListener {
